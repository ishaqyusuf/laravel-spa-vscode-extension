...
export function isLaravelSpaProject() {
  return (
    (fs.existsSync(join(Config.root, "vite.config.ts")) ||
      fs.existsSync(join(Config.root, "vite.config.js"))) &&
    fs.existsSync(join(Config.root, "composer.json"))
  );
}
...
export function loadComposerJSON() {
  const path = join(Config.root, "composer.json");
  if (fs.existsSync(path)) return JSON.parse(fs.readFileSync(path, "utf-8"));
}
...
export function getName(str: "vite" | "artisan") {
  if (str === "artisan") return "Laravel Spa";
  return "Vite";
}
...
export function hasVendors() {
  return fs.existsSync(join(Config.root, "vendors"));
}
