...
export async function showCommands() {
  const commands: CommandPickItem[] = [
    ...
    },
    {
      label:
        ctx.currentMode === "dev"
          ? `$(refresh) Restart ${getName(ctx.laravel.command)} server`
      ...
      async handler() {
        ...
        // if (reopen) await open({ browser: "embedded" });
      },
      if: ctx.active,
    },
    {
      label:
        ctx.active && ctx.currentMode === "serve"
